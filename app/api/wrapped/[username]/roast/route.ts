...
import { generateRoast } from "@/lib/server/ai";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const body = (await request.json()) as RoastBody;
    ...
    if (cachedWrapped?.roast) {
      return NextResponse.json({ roast: cachedWrapped.roast });
    }

    const roast = await generateRoast(body.stats);
